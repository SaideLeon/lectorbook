...
function TranscriptLine({ label, text, color }: { label: string; text: string; color: string }) {
  ...
  return (
    ...
    >
      <span className={cn('text-[10px] font-bold uppercase tracking-widest shrink-0 mt-0.5', color)}>{label}</span>
...
export function LiveVoicePanel({ contextFiles, apiKey, onBack }: LiveVoicePanelProps) {
  const {
    isActive, status, error, transcription,
  ...
  return (
    <div className="flex flex-col bg-[#111] rounded-xl border border-white/10 overflow-hidden h-[96%]">
      ...
      <div className="shrink-0 border-t border-white/8 bg-[#0f0f0f]">
        <div
          ref={transcriptRef}
          className="h-28 overflow-y-auto px-4 py-3 space-y-2 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent"
        >
          <AnimatePresence>
